...
/*   Updated: 2022/12/28 22:38:25 by anass_elaou      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	key_hook(int keycode, t_window *window)
{
	...
	if (keycode == A)
	{
		...
		window->line_x = window->y + (cos(window->rotation_angle) * 7) * 20.0;
		window->line_y = window->x + (sin(window->rotation_angle) * 7) * 20.0;
		minimap(window->map, window);
	}
	if (keycode == S)
	{
		window->y -= (sin(window->rotation_angle) * 7);
		window->x -= (cos(window->rotation_angle) * 7);
		minimap(window->map, window);
	}
	if (keycode == D)
	{
